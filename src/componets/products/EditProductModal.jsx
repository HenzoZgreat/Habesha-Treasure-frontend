...
const EditProductModal = ({ isOpen, onClose, onUpdateProduct, product, products }) => {
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      setErrors({ success: 'Product updated successfully!' });
      await onUpdateProduct();
      setTimeout(onClose, 2000);
    } catch (error) {
      const errorMessage = error.response?.data || 'Error updating product';
      ...
      <div className="bg-white shadow-xl rounded-xl p-6 w-full max-w-md max-h-[80vh] overflow-y-auto">
        <div className="flex items-center justify-between mb-4">
          <div className="flex items-center space-x-3">
            ...
            <h2 className="text-xl font-semibold text-1D3F93">Edit Product</h2>
          </div>
          <button onClick={onClose} className="text-gray-500 hover:text-gray-700">
            <FiX size={24} />
          </button>
        ...
        {errors.success && (
          <div className="mb-3 p-2 bg-green-100 border-l-4 border-green-500 text-green-700 text-sm">
            {errors.success}
          </div>
        )}
        {errors.general && (
          <div className="mb-3 p-2 bg-red-100 border-l-4 border-red-500 text-red-700 text-sm">
            {errors.general}
          </div>
        )}
        {Object.keys(errors).length > 0 && !errors.general && !errors.success && (
          <div className="mb-4 p-3 bg-gray-100 border-l-4 border-red-500 rounded-full text-sm text-red-700">
            <ul>
        ...
        <form onSubmit={handleSubmit} className="space-y-4">
          ...
          <TextAreaField label="Description (Amharic)" name="descriptionAmharic" value={formData.descriptionAm} onChange={handleChange} />
          ...
          <div className="sticky bottom-0 bg-white pt-4 border-t border-gray-200">
            <div className="flex justify-end space-x-2">
              ...
              <button
                type="submit"
                className={`px-4 py-2 bg-green-600 rounded-md hover:bg-green-700 text-white flex items-center ${saving ? 'opacity-50 cursor-not-allowed' : ''}`}
                disabled={saving}
              >
                {saving && <Spinner />}
